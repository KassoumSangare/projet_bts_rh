        ...
        <div class="col-lg-10">
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-header bg-primary text-white text-center fs-5 fw-semibold">
                    <i class="ri-calendar-add-line me-2"></i> Enregistrement d’un type de congé
                </div>

                <div class="card-body">
                    <form class="row g-4 needs-validation" method="POST" action="{{ route('conge.store') }}" novalidate>
                        @csrf
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="libelle" id="libelle" rows="4" class="form-control @error('libelle') is-invalid @enderror"
                                placeholder="Décrivez le type de congé en détail">{{ old('libelle') }}</textarea>
                            @error('libelle')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
                        </div>
